// <copyright file="Izh04BasicCodingTests.cs" company="PlaceholderCompany">
...
namespace Izh_04_Basic_Coding
{
    ...
    {
        public int InsertNumber(int x, int y, int i, int j)
        {
            var length = j - i + 1;
            var mask = 0;
        ...
        public int CheckMaxElementFinding(int[] sourceArray)
        {
            ...
            var copySourceArray = new int[sourceArray.Length - 1];
            Array.ConstrainedCopy(sourceArray, 1, copySourceArray, 0, copySourceArray.Length);
            return Math.Max(sourceArray[0], this.CheckMaxElementFinding(copySourceArray));
        }
        ...
        {
            int? hasEqualSum = null;
            var difference = .0001;
            var length = sourceArray.Length;
            if (length > 0)
            {
                var leftSum = new double[length];
                var rightSum = new double[length];
                leftSum[0] = sourceArray[0];
                rightSum[length - 1] = sourceArray[length - 1];
                for (int i = 1; i < sourceArray.Length; i++)
                {
                    leftSum[i] = leftSum[i - 1] + sourceArray[i];
                    rightSum[length - 1 - i] = rightSum[length - i] + sourceArray[length - 1 - i];
                }

                for (int i = 0; i < sourceArray.Length; i++)
                {
                    if (Math.Abs(leftSum[i] - rightSum[i]) <= difference)
                    {
                        hasEqualSum = i;
                    }
                }
        ...
        {
            // So, basically, if I understood from the test sample, I need to take the first string as a list of
            // chars that shouldn't be in the second string and then, concatenate them.
            var retStr = firstStr;

            foreach (char i in secondStr)
            {
        ...
        public bool HasDigit(int input, int digit)
        {
            while (Math.Abs(input) > 0)
            {
                if (Math.Abs(input) % 10 == digit)
                {
                    return true;
                }
        ...
        // This test case was wrong in that -69 does contain 9 in it.
        ...
        public int IntFromList(List<int> lInt)
        {
            int retval = 0;
            for (int i = 0; i < lInt.Count; i++)
            {
                retval += (int)(lInt[i] * Math.Pow(10, i));
            }

            return retval;
        }

        public void Swap(ref List<int> lInt, int x, int y)
        {
            var t = lInt[x];
            lInt[x] = lInt[y];
            lInt[y] = t;
        ...
        // Simple Bubble Sort on a subpart of lInt, from 0 to x-1
        public void SortPart(ref List<int> lInt, int x)
        {
            for (int i = 1; i < x; i++)
            {
                if (lInt[i] > lInt[i - 1])
        ...
        public int FindNextBiggerNumber(int input)
        {
            int? retval = null;
            var copyIn = input;

            // Decimal representation
            List<int> listDec = new List<int>();
            while (Math.Abs(copyIn) > 0)
            {
                listDec.Add(copyIn % 10);
                copyIn /= 10;
            }
            ...
            {
                for (int j = i + 1; j < listDec.Count; j++)
                {
                    int[] arrCopy = new int[listDec.Count];
                    listDec.CopyTo(arrCopy);
                    var listCopy = new List<int>(arrCopy);
                    this.Swap(ref listCopy, i, j);
                    this.SortPart(ref listCopy, j);
                    var tNum = this.IntFromList(listCopy);
                    if (tNum > input)
                    {
                        retval = (retval == null) ? tNum : Math.Min((int)retval, tNum);
                    }
                }
            }
        ...
        public Tuple<int, TimeSpan?> FindNextBiggerNumber(int input, bool giveTimeExecution)
        {
            var tStart = DateTime.Now;
            if (giveTimeExecution)
            {
                return new Tuple<int, TimeSpan?>(this.FindNextBiggerNumber(input), DateTime.Now.Subtract(tStart));
            }
            else
            {
                return new Tuple<int, TimeSpan?>(this.FindNextBiggerNumber(input), null);
            }
        }
    }
}
